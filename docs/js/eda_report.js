document.addEventListener("DOMContentLoaded", function () {
  ...
  if (edaData) {
    ...
    const descriptiveStatistics = edaData?.descriptive_statistics?.numeric_summary ?? {};

    // Get the headers dynamically from the first field's keys (assuming all fields have the same metrics)
    const headers = Object.keys(Object.values(descriptiveStatistics)[0] || {});
    ...
    const descriptiveStatisticsTable = `
      <table class="table table-bordered">
        ...
        <tbody>
          ${Object.entries(descriptiveStatistics).map(([field, metrics]) => `
            <tr>
              <td>${field}</td>
              ${headers.map(header => `<td>${metrics[header] ?? 'N/A'}</td>`).join("")}
            </tr>
          `).join("")}
        </tbody>
      </table>
  ...
  <table class="table table-bordered">
    ...
    <tbody>
      ${Object.entries(edaData?.descriptive_statistics?.categorical_summary ?? {}).map(
        ([category, values]) => `
          <tr>
            <td rowspan="${Object.keys(values).length + 1}"><strong>${category}</strong></td>
          </tr>
...
`;
    ...
    <div class="card shadow mb-4">
      ...
      <div class="card-body">
        ...
        <!-- Numeric Analysis -->
        <h4 class="card-header bg-dark text-white">Numeric Analysis</h4><br>
        ...
        <h4 class="card-header bg-dark text-white">Categorical Analysis</h4><br>
        ${Object.entries(edaData?.univariate_analysis?.categorical_barplots ?? {}).map(([category, plotBase64]) => `
          <div class="mb-4 border-bottom pb-4">
            <h5>${category} Bar Plot</h5>
            <img src="data:image/png;base64,${plotBase64}" alt="${category} Bar Plot" class="img-fluid mx-auto d-block">
          </div>
        `).join("")}
        ...
        <div class="row mb-4">
          <div class="col-12">
            <div class="card shadow">
              ...
              <div class="card-body">${categoricalSummaryTable}</div>
            </div>
          </div>
        </div>
